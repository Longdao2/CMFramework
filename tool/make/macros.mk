...
usr_define     =  $(eval DEF := $(strip $(1)))-$(strip $(2))$(DEF)$(if $(DEF_$(DEF)),="$(subst ",\",$(subst \,\\,$(DEF_$(DEF))))")
...
src_depend     =  if [ -d $(OUT_DIR) ]; then echo "SRC_PREV := $(subst ",\",$(ALL_SRC_FILES))" > $(PROJ_EXE:.exe=.d); fi
...
build_process  =  $(build_start) echo "$$($(call message_green, Compiled $<) & echo $(SH_SPACE) & \
